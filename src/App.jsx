...
export default function App() {
  ...
  async function handleCorrectionSubmit() {
    ...
    const result = await db.insert("Corrections", {
      ...
      current_value: correctionForm.currentValue, status: "pending", requested_on: getTodayStr()
    });
    if (Array.isArray(result) && result[0]) {
      setCorrections(prev => [...prev, result[0]]);
      setShowCorrectionModal(false);
      setCorrectionForm({ type: "attendance", date: "", note: "", currentValue: "" });
  ...
  async function handleCorrectionAction(id, action) {
    ...
    if (Array.isArray(result) && result[0]) {
      ...
      notify(action === "approved" ? "Correction approved " : "Correction rejected ");
    }
  }
